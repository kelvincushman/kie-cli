...
//go:embed market_catalog.json
var embeddedMarketCatalog []byte
...
func loadMarketCatalog() (marketCatalog, error) {
	var catalog marketCatalog
	if err := json.Unmarshal(embeddedMarketCatalog, &catalog); err != nil {
		return marketCatalog{}, fmt.Errorf("decoding embedded Market catalog: %w", err)
	}
	return catalog, nil
}
...
func newMediaModelsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "models [query]",
		Short: "List the embedded Kie standard Market task-model catalog",
		Long: "List the Kie Market task-model snapshot embedded in this CLI. It includes English Market pages that document the shared jobs/createTask task route; it is not a live or all-endpoint-family catalog. " +
			"Refresh the source tree with scripts/weekly-refresh.sh, then rebuild the CLI to pick up Kie's latest llms.txt index.",
		Args: cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			if err := validateDataSourceStrategy(flags, "local"); err != nil {
				return usageErr(err)
			}
			catalog, err := loadMarketCatalog()
			if err != nil {
				return err
			}
			query := strings.ToLower(strings.TrimSpace(family))
			if len(args) == 1 {
				query = strings.TrimSpace(query + " " + strings.ToLower(args[0]))
			}
			words := strings.Fields(query)
			models := make([]marketModel, 0, len(catalog.Models))
			for _, model := range catalog.Models {
				haystack := strings.ToLower(model.Category + " " + model.Summary + " " + model.Model)
				matches := true
	...
	cmd.Flags().StringVar(&family, "family", "", "Filter category, name, or model ID (for example: video or wan)")
	return cmd
}
...
func newMediaVideoCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Create one direct Market video task without a local brief or the director's preview gate. The built-in flags implement only Wan 2.7 text-to-video. " +
			"Select any other Kie Market video model with --model and pass its documented input object through --input.",
